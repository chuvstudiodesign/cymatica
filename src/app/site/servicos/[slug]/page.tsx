...
export async function generateMetadata(props: PageProps<"/site/servicos/[slug]">) {
...
export default async function ServicoPage(props: PageProps<"/site/servicos/[slug]">) {
  const { slug } = await props.params
  const service = services.find((s) => s.slug === slug)
  if (!service) notFound()

  const pricing = pricedServices.find((s) => s.slug === slug)
  // Projetos que usaram este serviço — prova em vez de promessa.
        ...
        <Container>
          ...
          <div className="mt-16 grid gap-16 lg:grid-cols-12">
            <div className="lg:col-span-7">
              ...
              <h1 className="site-h2 mt-8 text-balance">{service.name}</h1>
              <p className="site-lead mt-10 max-w-[52ch] text-pretty">
                {service.benefit}
              </p>
              <p className="site-lead mt-8 max-w-[56ch] text-pretty text-muted-foreground">
                {service.description}
              </p>
            </div>
            ...
            {pricing && (
              <div className="lg:col-span-5">
                <div className="rounded-2xl border border-border bg-card p-8">
                  ...
                  <Link
                    href={`/site/orcamento?servico=${service.slug}`}
