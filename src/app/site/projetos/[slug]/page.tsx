...
export async function generateMetadata(props: PageProps<"/site/projetos/[slug]">) {
...
export default async function ProjetoPage(props: PageProps<"/site/projetos/[slug]">) {
  const { slug } = await props.params
      ...
      <article className="pt-40 pb-28 md:pt-52 md:pb-40">
        <Container>
          ...
          <div className="mt-16 max-w-4xl">
            ...
            <h1 className="site-display mt-8">{project.client}</h1>
            <p className="site-lead mt-10 max-w-[52ch] text-pretty text-muted-foreground">
              {project.summary}
            </p>
          </div>

          <div className="relative mt-24 aspect-9/16 max-h-[80vh] overflow-hidden rounded-2xl bg-card sm:aspect-16/10">
            <Image
              src={project.cover}
          ...
          >
            <div>
              ...
              <p className="site-h2 mt-4">{next.client}</p>
            </div>
            <ArrowRight
